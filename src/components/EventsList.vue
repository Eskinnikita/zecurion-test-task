<template>
  ...
  >
    <v-list color="transparent">
      ...
      <template v-if="eventsStore.events.length">
        <v-list-item v-for="(event, index) in eventsStore.events" :key="index">
          ...
          <ul class="mb-1">
            ...
            <li v-if="getValue('dates', event.dateId, 'date')">
              <strong>Date: </strong
              >{{ getValue("dates", event.dateId, "date") }}
            </li>
          ...
          <v-divider
            v-if="index < eventsStore.events.length - 1"
            :key="index"
          />
        </v-list-item>
      </template>
      <div class="text-center mt-2 mb-2" v-else>Events list is empty</div>
    </v-list>
  </v-navigation-drawer>
</template>
<script setup>
